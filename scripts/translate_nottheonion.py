...
INPUT_FILE = SCRIPT_DIR / "nottheonion_top2000.ndjson"
OUTPUT_FILE = SCRIPT_DIR / "nottheonion_he.ndjson"
...
def main():
    ...
    with OUTPUT_FILE.open("a", encoding="utf-8") as out:
        for i, post in enumerate(remaining):
            try:
                translated = translator.translate(post["title"])
                entry = {"id": post["id"], "text": translated}
                out.write(json.dumps(entry, ensure_ascii=False) + "\n")
                out.flush()
                print(f"[{len(translated_ids) + i + 1}/{len(posts)}] {post['title'][:60]}")
