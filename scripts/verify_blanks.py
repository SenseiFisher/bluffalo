...
- **הסגידה**: המשפט לא יכול לרמוז על תוכן הבלנק. אם הניסוח במשפט מגלה את סוג הבלנק (למשל "קיום [blank]" מרמז שהבלנק הוא מעשה מיני), חובה לשכתב את המשפט כך שהבלנק יישאר מפתיע.
- **שפה יומיומית**: העדף מילים מדוברות ונפוצות על פני מונחים רשמיים כשהמשמעות זהה. למשל: "סקס" עדיף על "יחסי מין", "כסף" עדיף על "מטבע" בהקשר יומיומי.
    ...
    for i in range(0, len(batches), CONCURRENCY):
        ...
        for batch_idx, results in enumerate(all_results):
            ...
            for result in results:
                if result["id"] not in entries_by_id:
                    print(f"[WARN] Unknown ID from model: {result['id']}", file=sys.stderr)
                    continue
